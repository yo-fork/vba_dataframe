...
| 関数 | 説明 |
|---|---|
| `DF.FromRange(rng, [hasHeader])` | Excel Range から作成 |
| `DF.FromArray(data2D, colNames)` | 2D配列から作成 |
| `DF.Create(colNames, row1, row2, ...)` | 行データを直接指定 |
| `DF.FromCsv(filePath, [sep], [hasHeader])` | CSVファイルから作成 |
| `DF.EmptyFrame(colNames)` | 空の DataFrame を作成 |
